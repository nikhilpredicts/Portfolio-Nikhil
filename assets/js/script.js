...
const filterFunc = function (selectedValue) {
  ...
  for (let i = 0; i < filterItems.length; i++) {
    ...
    } else if (selectedValue === itemCategory || selectedValue.toLowerCase() === itemCategory.toLowerCase()) {
      filterItems[i].classList.add("active");
    } else {
...
for (let navIndex = 0; navIndex < navigationLinks.length; navIndex++) {
  navigationLinks[navIndex].addEventListener("click", function () {
    ...
    for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
      if (this.innerHTML.trim().toLowerCase() === pages[pageIndex].dataset.page) {
        pages[pageIndex].classList.add("active");
        navigationLinks[navIndex].classList.add("active"); // ✅ Now correct link gets highlighted
        window.scrollTo(0, 0);
      } else {
